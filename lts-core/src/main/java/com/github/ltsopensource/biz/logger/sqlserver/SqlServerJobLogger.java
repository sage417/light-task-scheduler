 ...
 */
public class SqlServerJobLogger extends JdbcAbstractAccess implements JobLogger {

    public SqlServerJobLogger(Config config) {
        super(config);
        createTable(readSqlFile("sql/sqlserver/lts_job_log_po.sql"));
    }
    ...
    private InsertSql buildInsertSql() {
        return new InsertSql(getSqlTemplate()).setEscape(SqlServerEscape.Holder.instance)
                .insert(getTableName())
                .columns("log_time",
                        "gmt_created",
                        "log_type",
