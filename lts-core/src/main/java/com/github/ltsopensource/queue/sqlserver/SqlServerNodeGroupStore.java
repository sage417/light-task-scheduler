 ...
 */
public class SqlServerNodeGroupStore extends JdbcAbstractAccess implements NodeGroupStore {

    public SqlServerNodeGroupStore(Config config) {
        super(config);
        createTable(readSqlFile("sql/sqlserver/lts_node_group_store.sql", JobQueueUtils.NODE_GROUP_STORE));
    }

    @Override
    public void addNodeGroup(NodeType nodeType, String name) {

        Integer count = new SelectSql(getSqlTemplate()).setEscape(SqlServerEscape.Holder.instance)
                .select()
                .columns("count(1)")
                .from()
                .table(getTableName())
