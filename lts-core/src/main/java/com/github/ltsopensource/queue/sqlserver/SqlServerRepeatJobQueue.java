 ...
 */
public class SqlServerRepeatJobQueue extends SqlServerSchedulerJobQueue implements RepeatJobQueue {

    public SqlServerRepeatJobQueue(Config config) {
        super(config);
        createTable(readSqlFile("sql/sqlserver/lts_repeat_job_queue.sql", getTableName()));
    }
    ...
    public int incRepeatedCount(String jobId) {
        while (true) {
            ...
            if (new UpdateSql(getSqlTemplate()).setEscape(SqlServerEscape.Holder.instance)
                    .update()
                    .table(getTableName())
