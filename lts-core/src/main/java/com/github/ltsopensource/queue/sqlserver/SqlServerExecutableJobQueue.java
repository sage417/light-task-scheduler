 ...
 */
public class SqlServerExecutableJobQueue extends AbstractSqlServerJobQueue implements ExecutableJobQueue {

    public SqlServerExecutableJobQueue(Config config) {
        super(config);
    }
    ...
    public boolean createQueue(String taskTrackerNodeGroup) {
        createTable(readSqlFile("sql/sqlserver/lts_executable_job_queue.sql", getTableName(taskTrackerNodeGroup)));
        return true;
    }
    ...
    public long countJob(String realTaskId, String taskTrackerNodeGroup) {
        return (Long) new SelectSql(getSqlTemplate()).setEscape(SqlServerEscape.Holder.instance)
                .select()
                .columns("COUNT(1)")
                .from()
                .table(getTableName(taskTrackerNodeGroup))
    ...
    public void resume(JobPo jobPo) {
        new UpdateSql(getSqlTemplate()).setEscape(SqlServerEscape.Holder.instance)
                .update()
