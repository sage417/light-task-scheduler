 ...
 */
public class SqlServerPreLoader extends AbstractPreLoader {

    private static final Logger LOGGER = LoggerFactory.getLogger(SqlServerPreLoader.class);
    private SqlTemplate sqlTemplate;

    public SqlServerPreLoader(AppContext appContext) {
        super(appContext);
    ...
    protected JobPo getJob(String taskTrackerNodeGroup, String jobId) {
        return new SelectSql(sqlTemplate).setEscape(SqlServerEscape.Holder.instance)
                .select()
                .all()
                .from()
    ...
    protected boolean lockJob(String taskTrackerNodeGroup, String jobId,
                              ...
                              Long gmtModified) {
        try {
            return new UpdateSql(sqlTemplate).setEscape(SqlServerEscape.Holder.instance)
                    .update()
                    .table(getTableName(taskTrackerNodeGroup))
                    .set("is_running", true)
